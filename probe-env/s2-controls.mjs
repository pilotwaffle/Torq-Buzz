...
for (let round = 1; round <= rounds; round++) {
  await withPage(async (c) => { await evalJs(c, `(() => { document.querySelector('[data-testid=message-input]').focus(); return true; })()`); await typeEnter(c, prompt); });
  console.log(new Date().toISOString(), `round ${round}: prompt sent`);
  // open the session panel (composer activity chip) once the agent is working
  for (let i = 0; i < 15; i++) {
    await wait(2000);
  ...
  await wait(1500);
  await control("Steer agent with a message", "steer", async (c) => { await wait(500); await evalJs(c, `(() => { const t=[...document.querySelectorAll('textarea')].find(x=>x.getAttribute('aria-label')==='Steer message'); t.focus(); return true; })()`); await typeCtrlEnter(c, `Steer ${round}: stop what you are doing and summarize your progress in two sentences.`); });
  await wait(1500);
